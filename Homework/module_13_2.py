...
#
# Напишите асинхронную функцию start_strongman(name, power), где name -
# имя силача, power - его подъёмная мощность.
# Реализуйте следующую логику в функции:
#
# 1. В начале работы должна выводиться строка -
# 'Силач <имя силача> начал соревнования.'
# 2. После должна выводиться строка -
# 'Силач <имя силача> поднял <номер шара>' с задержкой обратно
# пропорциональной его силе power.
# Для каждого участника количество шаров одинаковое - 5.
# 3. В конце поднятия всех шаров должна выводится строка
# 'Силач <имя силача> закончил соревнования.'
#
# Также напишите асинхронную функцию start_tournament, в которой создаются
# 3 задачи для функций start_strongman.
# Имена(name) и силу(power) для вызовов функции start_strongman можете выбрать
# самостоятельно.
#
...
lifting_time = 1
strongmen = [('Pasha', 3), ('Denis', 4), ('Apollon', 5)]


async def start_strongman(name, power):
    print(f'Силач {name} начал соревнования.')
    for i in range(5):
        await asyncio.sleep(lifting_time / power)
        print(f'Силач {name} поднял {i + 1} шар.')
    print(f'Силач {name} закончил соревнования.')


async def start_tournament(strongmen):
    strongmen = [asyncio.create_task(start_strongman(*strongman))
